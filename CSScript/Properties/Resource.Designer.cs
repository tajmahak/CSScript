...
namespace CSScript.Properties {
    ...
    internal class Resource {
        ...
        /// ``[-h[ide]]  `i`- выполнение скрипта в скрытом режиме;
        /// ``[-p[ause]] `i`- после окончания работы скрипта ожидать от пользователя подтверждение закрытия;
        /// ``[-a[rgs] {...} [{...} ...]] `i`- входящие аргументы для использования в скрипте (args: string[]);
        /// ``[-reg]     `i`- регистрация расширения .cssc в реестре;
        /// ``[-unreg]   `i`- удаление расширения .cssc из реестра;
        ///
        ///`c`Служебный синтакси [остаток строки не уместился]&quot;;.
        /// </summary>
